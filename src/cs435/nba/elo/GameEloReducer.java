...
public class GameEloReducer extends Reducer<IntWritable, GameWritable, IntWritable, Text> {
	...
	public void reduce(IntWritable key, Iterable<GameWritable> values, Context context)
		...
		// Need to iterate over entire set, extract games and extract player elo values
		...
		// Use an arrayList to keep track of games because we can sort this before calculating Elo
		List<GameWritable> sortedGames = new ArrayList<GameWritable>(); 
		for( GameWritable game : values ){
			
			// Add the game to the list of games
			// Use copy constructor instead of object reference as it will disappear once iterated over
			sortedGames.add(new GameWritable(game));
			
			// Get the players from the game and add the to the playerEloMap, if they don't already exist
			try{
		...
		for( GameWritable game : sortedGames ){
			...
			// 6. Update the elo value for the players in the playerEloMap so we get correct values for the next game
			...
			try{
				...
				// Home Team
				TeamGameWritable homeTeam = game.getHomeTeam();
				str += "," + homeTeam.getTeamId() + "," + homeTeam.getPoints() + "," + homeTeam.getStartElo() + "," + homeTeam.getEndElo();
				
				// Away Team
				TeamGameWritable awayTeam = game.getAwayTeam();
				str += "," + awayTeam.getTeamId() + "," + awayTeam.getPoints() + "," + awayTeam.getStartElo() + "," + awayTeam.getEndElo();
