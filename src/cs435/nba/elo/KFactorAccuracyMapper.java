...
public class KFactorAccuracyMapper extends Mapper<LongWritable, Text, DoubleWritable, IntWritable> {
	...
	public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
		...
		if (homeTeamStartElo > awayTeamStartElo) {
			...
			if (homeTeamPoints > awayTeamPoints) {
				...
				// Correctly predicted
				context.write(new DoubleWritable(kFactor), new IntWritable(1));

			} else {

				// Incorrect prediction
				context.write(new DoubleWritable(kFactor), new IntWritable(0));
			}
